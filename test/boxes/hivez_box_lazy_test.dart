...
void main() {
  late HivezLazyBox<int, int> hivezBox;
  ...
  setUp(() async {
    hivezBox = HivezLazyBox<int, int>('lazyBoxTest');
    await hivezBox.ensureInitialized();
    await hivezBox.clear();
  });

  tearDownAll(() async {
    await Hive.deleteBoxFromDisk('lazyBoxTest');
  });
  ...
  group('String key/value + backup/restore (lazy box)', () {
    late HivezLazyBox<String, String> strBox;

    setUp(() async {
      strBox = HivezLazyBox<String, String>('lazyBoxTestStr');
      await strBox.ensureInitialized();
      await strBox.clear();
    });

    tearDown(() async {
      await Hive.deleteBoxFromDisk('lazyBoxTestStr');
    });
