...
abstract class AbstractHivezIsolatedBox<K, T, B extends IsolatedBoxBase<T>>
  ...
  Future<bool> containsKey(K key) async {
    return synchronizedRead(() => box.containsKey(key));
  }

  @override
  Future<int> get length async {
    return synchronizedRead(() => box.length);
  }
  ...
  Future<K> keyAt(int index) async {
    return synchronizedRead(() async => (await box.keyAt(index)) as K);
  }

  @override
  Future<bool> get isEmpty async {
    return synchronizedRead(() => box.isEmpty);
  }

  @override
  Future<bool> get isNotEmpty async {
    return synchronizedRead(() => box.isNotEmpty);
  }
