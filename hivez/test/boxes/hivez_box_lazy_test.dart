...
void main() {
  late HivezBoxLazy<int, int> hivezBox;
  ...
  setUp(() async {
    hivezBox = HivezBoxLazy<int, int>('lazyBoxTest');
  ...
  test('firstWhereContains performs case-insensitive substring search',
      () async {
    final box = HivezBoxLazy<int, String>('lazyTextBox');
    ...
    await Hive.deleteBoxFromDisk('lazyTextBox');
  });
  ...
  test('deleteFromDisk removes box data from disk', () async {
    final box = HivezBoxLazy<int, int>('lazyDiskBox');
    await box.ensureInitialized();
    await box.clear();
    ...
    final box2 = HivezBoxLazy<int, int>('lazyDiskBox');
